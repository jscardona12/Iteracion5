...
public class DAOPuerto {
	...
	public ArrayList<Puerto> darPuertos() throws SQLException, Exception 
	{
		...
		while (rs.next()) 
		{
			...
			puertos.add(new Puerto(name, c, id));
		}
		return puertos;
	}
	 ...
	 * @throws Exception - Cualquier error que no corresponda a la base de datos
	 */
	public ArrayList<Puerto> buscarPuertoPorName(String name) throws SQLException, Exception {
		
		ArrayList<Puerto> puertos = new ArrayList<Puerto>();

		String sql = "SELECT * FROM PUERTOS WHERE NAME ='" + name + "'";
		...
		while (rs.next()) 
		{
			...
			puertos.add(new Puerto(name2, c, id));
		}
	 ...
	 * @param puerto - el barco a actualizar. barco !=  null
	 ...
	 * @param puerto - el video a borrar. video !=  null
